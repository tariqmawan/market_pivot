  ...
  return (
    <div className="page region-detail">
      <section className="coverage-hero region-hero">
        <div>
          <Link to="/regions" className="back-link">{t("regionspagenew.h9")}</Link>
          <p className="eyebrow">{region.type} • {region.countries.length} countries</p>
          <h1>{detail.name}</h1>
          <p>{region.summary}</p>
        </div>
        <div className="metric-strip region-metrics">
          <div className="metric-tile"><span>{t("regionspagenew.h10")}</span><strong>{formatMoney(detail.gdp)}</strong></div>
          <div className="metric-tile"><span>{t("regionspagenew.h11")}</span><strong className="positive">{detail.gdpGrowth.toFixed(1)}%</strong></div>
          <div className="metric-tile"><span>{t("regionspagenew.h12")}</span><strong>{detail.inflation.toFixed(1)}%</strong></div>
          <div className="metric-tile"><span>{t("regionspagenew.h13")}</span><strong>{detail.population}</strong></div>
        </div>
      </section>
