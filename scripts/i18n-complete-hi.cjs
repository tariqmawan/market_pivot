...
async function main() {
  ...
  for (const namespace of NAMESPACES) {
    ...
    const targetPath = path.join(LOCALE_ROOT, "hi", `${namespace}.json`);
  ...
  console.log(`Hindi locale completion done. added=${counters.added} translated=${counters.translated} cache=${cache.size}`);
}
